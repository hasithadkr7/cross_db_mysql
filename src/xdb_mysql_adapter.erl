...
%% Adapter callbacks
-export([child_spec/2, insert/4, insert_all/4, update/5, delete/4, execute/5]).
...
insert(Repo, #{schema := Schema} = Meta, Fields0, _Opts) ->
    ...
    case get(self()) of
        undefined ->
            poolboy:transaction(pool_name(Repo),
                                fun(Worker) -> do_insert(Worker, Meta, Fields) end);
        Pid ->
...
insert_all(Repo, #{schema := Schema} = Meta, List0, _Opts) ->
    ...
    case get(self()) of
        undefined ->
            poolboy:transaction(pool_name(Repo),
                                fun(Worker) -> do_insert_all(Worker, Meta, List) end);
        Pid ->
...
update(Repo, #{schema := Schema, source := Source}, Fields, Filters, _Opts) ->
    ...
    case get(self()) of
        undefined ->
            poolboy:transaction(pool_name(Repo),
                                fun(Worker) -> do_update(Worker, Filters, Schema, Source, Query)
                                end);
        Pid ->
            do_update(Pid, Filters, Schema, Source, Query)
    end.
...
delete(Repo, #{schema := Schema, source := Source}, Filters, _Opts) ->
    ...
    case get(self()) of
        undefined ->
            poolboy:transaction(pool_name(Repo),
                                fun(Worker) -> do_delete(Worker, Filters, Schema, Source, Query)
                                end);
        Pid ->
            do_delete(Pid, Filters, Schema, Source, Query)
    end.
...
execute(Repo, Op, #{schema := Schema, source := Source}, Query, Opts) ->
    case get(self()) of
        undefined ->
            poolboy:transaction(pool_name(Repo),
                                fun(Worker) -> do_execute(Worker, Op, Schema, Source, Query, Opts)
                                end);
        Pid ->
...
transaction(Repo, Fun, Opts) ->
    ...
    poolboy:transaction(pool_name(Repo),
                        fun(Worker) ->
                           _ = put(self(), Worker),
                           Res = case mysql:transaction(Worker, TxFun) of
                                     {atomic, Result} ->
                                         {ok, Result};
...
in_transaction(Repo) ->
    case get(self()) of
        undefined ->
            poolboy:transaction(pool_name(Repo), fun(Worker) -> mysql:in_transaction(Worker) end);
        Pid ->
...
do_execute(Pid, all, Schema, Source, Query, Opts) ->
    ...
    {Query0, Params} =
        xdb_sql:s(Source, SelectFields, Conditions, [], Offset, Limit, OrderBy),
...
do_execute(Pid, delete_all, _Schema, Source, Query, _Opts) ->
    ...
    DQuery =
        case Limit of
            ...
            _ ->
                lists:flatten(DQuery0) ++ " LIMIT " ++ integer_to_list(Limit)
        end,
...
do_update(Pid, Filters, Schema, Source, #{updates := []}) ->
                             ...
                             _ ->
                                 case mysql:query(Pid, lists:flatten(UQuery), UParams ++ WParams) of
                                     ok ->
                                         GetQuery = get_query(Filters),
                                         {_, [Result]} =
                                             do_execute(Pid, all, Schema, Source, GetQuery),
                                         {ok, Result};
                                     {error, {Code, _SqlState, Reason}} ->
                                         Description =
                                             <<"Error with Code:",
...
do_count(Pid, CountQuery, CountParams) ->
    case mysql:query(Pid, lists:flatten(CountQuery), CountParams) of
        ...
        {error, {Code0, _SqlState0, Reason0}} ->
            Description0 =
                <<"Error with Code:",
                  (integer_to_binary(Code0))/binary,
                  " and Reason:",
                  Reason0/binary>>,
