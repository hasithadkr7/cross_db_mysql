...
%% Test Cases
-export([t_pool/1, t_insert_all/1, t_insert_errors/1, t_insert_autoincrement/1,
         t_update_empty/1]).
...
t_insert_errors(Config) ->
    ...
    UpdateCS = xdb_changeset:change(InitCS, #{first_name => <<"Joe">>}),
    ErrorCS = xdb_changeset:add_error(UpdateCS, first_name, <<"Invalid">>),
    {error, CS} = Repo:insert(ErrorCS),
